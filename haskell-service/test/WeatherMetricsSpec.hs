...
spec = do
  ...
  describe "safeStd" $ do
    ...
    it "std of [2,4,4,4,5,5,7,9] ≈ 2.0 (sample std)" $
      safeStd [2,4,4,4,5,5,7,9] `shouldSatisfy` \ms ->
        case ms of Just v -> inRange 1.9 2.1 v; Nothing -> False
  ...
  describe "raFAO" $ do
    ...
    it "reasonable range for lat=47.5 doy=120 (15–40 MJ/m²/day)" $
  ...
  describe "et0FAO56" $ do
    ...
    it "zero when VPD = 0 and Rn ≤ 0 (cold humid overcast)" $
  ...
  describe "gddFromDays" $ do
    ...
    it "mixed: only days with mean > 10 contribute" $ do
      let coldDay = DaySummary { dsTmin=2.0,  dsTmax=8.0,  dsTmean=5.0
          ...
          -- warm contributes (24+16)/2 - 10 = 11 each × 2 = 22
  ...
  describe "spiApprox" $ do
    ...
    it "returns a finite non-zero value for variable rain" $
  ...
  describe "computeMetrics - water deficit" $ do
    ...
    it "water_deficit_7d = Σ daily_et0(7d) - (rain7 + snow7)" $ do
