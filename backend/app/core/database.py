...
class FieldWork(Base):
    ...
    equipment_usage = relationship("EquipmentUsageLog", back_populates="field_work")
...
# =============================================================================
# Equipment / Machinery  (eGN-aligned)
# =============================================================================
...
class Equipment(Base):
    ...
    registration_plate = Column(String(64), nullable=True)
    inventory_number = Column(String(64), nullable=True)  # internal farm number

    # Technical specs
    power_kw = Column(Numeric(7, 1), nullable=True)  # engine power
    working_width_m = Column(Numeric(6, 2), nullable=True)  # implement working width
    tank_capacity_l = Column(Numeric(8, 1), nullable=True)  # fuel / spray tank
    fuel_type = Column(Enum(FuelType), nullable=True, default=FuelType.DIESEL)
    weight_kg = Column(Numeric(9, 1), nullable=True)

    # Operating hours tracking
    hours_initial = Column(Numeric(10, 1), nullable=True, default=0)  # hours at registration
    hours_current = Column(Numeric(10, 1), nullable=True, default=0)  # latest reading
    hours_service_interval = Column(Numeric(8, 1), nullable=True)  # e.g. 250 h between services

    # Status & ownership
    status = Column(Enum(EquipmentStatus), nullable=False,
                    default=EquipmentStatus.OPERATIONAL, index=True)
    is_owned = Column(Boolean, default=True, nullable=False)  # False = rented / contracted
    ...
    __table_args__ = (
        Index("ix_equipment_user_type", "user_id", "equipment_type"),
        Index("ix_equipment_user_status", "user_id", "status"),
    )
...
class EquipmentMaintenance(Base):
    ...
    hours_at_service = Column(Numeric(10, 1), nullable=True)  # odometer reading
    cost = Column(Numeric(10, 2), nullable=True)  # total cost (€)
    parts_cost = Column(Numeric(10, 2), nullable=True)
    labour_cost = Column(Numeric(10, 2), nullable=True)

    performed_by = Column(String(200), nullable=True)  # technician / workshop
    invoice_ref = Column(String(128), nullable=True)
    next_service_hours = Column(Numeric(10, 1), nullable=True)  # next due at X hours
    ...
    __table_args__ = (
        Index("ix_maint_equipment_date", "equipment_id", "maintenance_date"),
        Index("ix_maint_user_date", "user_id", "maintenance_date"),
    )
...
class EquipmentUsageLog(Base):
    ...
    # Optional link to field_work and field
    field_work_id = Column(Integer, ForeignKey("field_work.id"), nullable=True, index=True)
    field_id = Column(Integer, ForeignKey("field_units.id"), nullable=True, index=True)

    used_date = Column(Date, nullable=False, index=True)

    # Hours
    hours_start = Column(Numeric(10, 1), nullable=True)  # odometer at job start
    hours_end = Column(Numeric(10, 1), nullable=True)  # odometer at job end
    hours_worked = Column(Numeric(8, 1), nullable=True)  # computed or manually entered

    # Area / distance
    area_ha = Column(Numeric(10, 2), nullable=True)  # area covered this session
    distance_km = Column(Numeric(8, 1), nullable=True)  # for transport / haulage

    # Fuel
    fuel_consumed_l = Column(Numeric(8, 2), nullable=True)
    ...
    field_work = relationship("FieldWork", back_populates="equipment_usage")
    field = relationship("FieldUnit")
