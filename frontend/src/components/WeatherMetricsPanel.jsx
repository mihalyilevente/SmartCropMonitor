 ...
 *
 * Consumes:
 *   GET /api/v1/weather/location/{id}/latest-weather  → { history, metrics }
 *   GET /api/v1/weather/location/{id}/weather-stats   → { history, metrics, record_count }
 *
 */
  ...
  return (
    <div style={{ marginTop: 10 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 9, color: '#bbb', marginTop: 4 }}>
        <span>min {r1(min)}</span>
        <span>max {r1(max)}</span>
      </div>
    </div>
  );
};
  ...
  return (
    <div style={{ paddingTop: 10, borderTop: '1px solid #f0ebe3', marginTop: 8 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        {[['Historical avg', avg], ['Historical min', min], ['Historical max', max], ['Std deviation', std]].map(([label, val]) => (
    ...
    <>
      <div
        style={{
          ...
          border: expanded
            ? `1.5px solid ${color}`
            : hovered ? `1px solid ${color}88` : '1px solid #ece6dc',
          borderRadius: 10,
          padding: '10px 14px 28px 14px', // bottom padding makes room for the chip
          display: 'flex', flexDirection: 'column', gap: 2,
          minWidth: 130, position: 'relative',
          cursor: 'pointer',
          transition: 'border-color 0.15s, box-shadow 0.15s, transform 0.1s',
          boxShadow: expanded
            ? `0 0 0 3px ${color}22, 0 2px 12px rgba(0,0,0,0.08)`
            : hovered ? '0 2px 10px rgba(0,0,0,0.07)' : 'none',
      ...
      >
        ...
        {/* Avg hint — visible when collapsed */}
        ...
        {/* 📊 expand chip — always visible, bottom-right */}
        <div style={{
...
const RecordBadge = ({ count }) => {
  ...
  const label = count >= 8760 ? `${(count / 8760).toFixed(1)} yrs`
    : count >= 720 ? `${(count / 720).toFixed(1)} mo`
    : `${count} records`;
  return (
            ...
            <>
              ...
              {h && (
                <Section title="Current conditions">
                  <Stat icon="🌡️" label="Temperature"       value={fmt(h.temp)}                    unit="°C"    color="#b53060" metricKey="temp"                    statEntry={hs('temp')}                    {...alertProps} {...statsProps} />
                  <Stat icon="💧" label="Humidity"           value={fmt(h.humidity, 0)}             unit="%"     color="#1a6fa3" metricKey="humidity"                statEntry={hs('humidity')}                {...alertProps} {...statsProps} />
                  <Stat icon="🌬️" label="Wind speed"         value={fmt(h.wind_speed)}              unit="m/s"   color="#4a5568" metricKey="wind_speed"              statEntry={hs('wind_speed')}              {...alertProps} {...statsProps}
                    sub={h.wind_deg != null ? `${h.wind_deg}°` : undefined} />
                  <Stat icon="☁️" label="Cloud cover"        value={fmt(h.cloud_coverage, 0)}       unit="%"     color="#7a8fa0" metricKey="cloud_coverage"           statEntry={hs('cloud_coverage')}          {...alertProps} {...statsProps} />
                  <Stat icon="🌧️" label="Precipitation"      value={fmt(h.precipitation)}           unit="mm"    color="#1a7a6e" metricKey="precipitation"           statEntry={hs('precipitation')}           {...alertProps} {...statsProps} />
                  <Stat icon="🌦️" label="Rain"               value={fmt(h.rain)}                    unit="mm"    color="#1a7a6e" metricKey="rain"                    statEntry={hs('rain')}                    {...alertProps} {...statsProps} />
                  <Stat icon="🌨️" label="Snowfall"           value={fmt(h.snowfall)}                unit="cm"    color="#5577a0" metricKey="snowfall"                                                          {...alertProps} {...statsProps} />
                  <Stat icon="⬇️" label="Pressure"           value={fmt(h.pressure, 1)}             unit="hPa"   color="#47637a" metricKey="pressure"               statEntry={hs('pressure')}                {...alertProps} {...statsProps} />
                  <Stat icon="🌫️" label="Dew point"          value={fmt(h.dew_point)}               unit="°C"    color="#8b6340" metricKey="dew_point"              statEntry={hs('dew_point')}               {...alertProps} {...statsProps} />
                  <Stat icon="💨" label="Vapour press. def." value={fmt(h.vapour_pressure_deficit)} unit="kPa"   color="#6b7a4a" metricKey="vapour_pressure_deficit" statEntry={hs('vapour_pressure_deficit')} {...alertProps} {...statsProps} />
                </Section>
              )}

              {h && (
                <Section title="Soil">
                  <Stat icon="🌱" label="Soil temp (0 cm)"       value={fmt(h.soil_temperature_0cm)}       unit="°C"    color="#8b6340" metricKey="soil_temperature_0cm"   statEntry={hs('soil_temperature_0cm')}   {...alertProps} {...statsProps} />
                  <Stat icon="🪱" label="Soil moisture (0–1 cm)"  value={fmt(h.soil_moisture_0_to_1cm, 3)}  unit="m³/m³" color="#317f43" metricKey="soil_moisture_0_to_1cm" statEntry={hs('soil_moisture_0_to_1cm')} {...alertProps} {...statsProps} />
                </Section>
              )}

              {m && (
                <Section title="7-day agronomic summary">
                  <Stat icon="🌡️" label="Temp max (day)"    value={fmt(m.temp_max_day_7d)}         unit="°C"    color="#b53060" metricKey="temp_max_day_7d"       statEntry={ms('temp_max_day_7d')}       {...alertProps} {...statsProps} />
                  <Stat icon="🌙" label="Temp min (night)"  value={fmt(m.temp_min_night_7d)}       unit="°C"    color="#1a6fa3" metricKey="temp_min_night_7d"     statEntry={ms('temp_min_night_7d')}     {...alertProps} {...statsProps} />
                  <Stat icon="🌤️" label="Temp min (day)"    value={fmt(m.temp_min_day_7d)}         unit="°C"    color="#47637a" metricKey="temp_min_day_7d"       statEntry={ms('temp_min_day_7d')}       {...alertProps} {...statsProps} />
                  <Stat icon="🔥" label="Temp max (night)"  value={fmt(m.temp_max_night_7d)}       unit="°C"    color="#b87300" metricKey="temp_max_night_7d"     statEntry={ms('temp_max_night_7d')}     {...alertProps} {...statsProps} />
                  <Stat icon="🌧️" label="Rain 7d"           value={fmt(m.rain_cum_7d)}             unit="mm"    color="#1a7a6e" metricKey="rain_cum_7d"           statEntry={ms('rain_cum_7d')}           {...alertProps} {...statsProps} />
                  <Stat icon="💦" label="ET₀"               value={fmt(m.et0)}                     unit="mm"    color="#317f43" metricKey="et0"                   statEntry={ms('et0')}                   {...alertProps} {...statsProps} />
                  <Stat icon="📉" label="Water deficit 7d"  value={fmt(m.water_deficit_7d)}        unit="mm"    color="#470736" metricKey="water_deficit_7d"      statEntry={ms('water_deficit_7d')}      {...alertProps} {...statsProps} />
                  <Stat icon="💧" label="Humidity mean 7d"  value={fmt(m.humidity_mean_7d)}        unit="%"     color="#1a6fa3" metricKey="humidity_mean_7d"      statEntry={ms('humidity_mean_7d')}      {...alertProps} {...statsProps} />
                  <Stat icon="🥶" label="Frost days 7d"     value={fmt(m.frost_days_count_7d, 0)}  unit="d"     color="#5577a0" metricKey="frost_days_count_7d"   statEntry={ms('frost_days_count_7d')}   {...alertProps} {...statsProps} />
                  <Stat icon="🌞" label="Heat days 7d"      value={fmt(m.heat_days_count_7d, 0)}   unit="d"     color="#c0392b" metricKey="heat_days_count_7d"    statEntry={ms('heat_days_count_7d')}    {...alertProps} {...statsProps} />
                  <Stat icon="☀️" label="Solar rad. (Rs)"   value={fmt(m.rs_mj_m2_day)}            unit="MJ/m²" color="#b87300" metricKey="rs_mj_m2_day"         statEntry={ms('rs_mj_m2_day')}          {...alertProps} {...statsProps} />
                  <Stat icon="🌐" label="Extraterr. rad."   value={fmt(m.ra_mj_m2_day)}            unit="MJ/m²" color="#d4a017" metricKey="ra_mj_m2_day"         statEntry={ms('ra_mj_m2_day')}          {...alertProps} {...statsProps} />
                  <Stat icon="🌱" label="GDD (base 10)"     value={fmt(m.gdd_base_10, 1)}          unit="°C·d"  color="#317f43" metricKey="gdd_base_10"          statEntry={ms('gdd_base_10')}           {...alertProps} {...statsProps} />
                </Section>
              )}

              {m && (
                <Section title="30-day summary">
                  <Stat icon="🌧️" label="Rain 30d"           value={fmt(m.rain_cum_30d)}              unit="mm"  color="#1a7a6e" metricKey="rain_cum_30d"          statEntry={ms('rain_cum_30d')}          {...alertProps} {...statsProps} />
                  <Stat icon="📉" label="Water deficit 30d"  value={fmt(m.water_deficit_30d)}         unit="mm"  color="#470736" metricKey="water_deficit_30d"     statEntry={ms('water_deficit_30d')}     {...alertProps} {...statsProps} />
                  <Stat icon="💧" label="Humidity mean 30d"  value={fmt(m.humidity_mean_30d)}         unit="%"   color="#1a6fa3" metricKey="humidity_mean_30d"     statEntry={ms('humidity_mean_30d')}     {...alertProps} {...statsProps} />
                  <Stat icon="🥶" label="Frost days 30d"     value={fmt(m.frost_days_count_30d, 0)}   unit="d"   color="#5577a0" metricKey="frost_days_count_30d"  statEntry={ms('frost_days_count_30d')}  {...alertProps} {...statsProps} />
                  <Stat icon="🔥" label="Heat days 30d"      value={fmt(m.heat_days_count_30d, 0)}    unit="d"   color="#b53060" metricKey="heat_days_count_30d"   statEntry={ms('heat_days_count_30d')}   {...alertProps} {...statsProps} />
                  <Stat icon="🌱" label="SPI 1m"             value={fmt(m.spi_1m, 2)}                 unit=""    color="#317f43" metricKey="spi_1m"                statEntry={ms('spi_1m')}                {...alertProps} {...statsProps} />
                </Section>
              )}
            </>
          )}
