/**
 * FieldMapPanel.jsx
 * Mapbox GL JS panel showing field boundaries + metric heatmap overlay.
 * Sits above SensorPanel in Dashboard.
 *
 * Props:
 *   userId      {number}
 *   locationId  {number}
...
// Convert plotly z/x/y grid to GeoJSON points for heatmap
...
const FieldMapPanel = ({ userId, locationId }) => {
  ...
  const [fields, setFields]         = useState(null);   // GeoJSON FeatureCollection
  ...
  const loadMetric = useCallback(() => {
    ...
    setMetricError(null);
    api.get(`/api/v1/location/${locationId}/latest-metrics/${metric}`, { params: { user_id: userId } })
  ...
  useEffect(() => {
    ...
    if (map.getSource(SRC)) {
      ...
      map.addLayer({
        ...
        paint: {
          'fill-color': ['case',
            ['==', ['get', 'field_type'], 'crop'], 'rgba(134,197,75,0.18)',
  ...
  useEffect(() => {
    ...
    }, 'fields-fill'); // insert below field layers
  }, [mapReady, metricData, metric]);
    ...
    <div style={styles.panel}>
      ...
      <div style={styles.panelHeader} onClick={() => setOpen(v => !v)}>
        <div style={styles.panelTitle}>
          ...
          {fields && (
            <span style={styles.badge}>{fields.features.length} field{fields.features.length !== 1 ? 's' : ''}</span>
          )}
        </div>
        <div style={styles.panelRight}>
          {metricLoading && <span style={styles.loadingDot} />}
        ...
        <div style={styles.panelBody}>
          ...
          <div style={styles.mapWrap}>
            ...
            <div style={styles.legend}>
              <div style={styles.legendTitle}>{meta.label} <span style={styles.legendDesc}>{meta.desc}</span></div>
...
const styles = {
  ...
  panelBody: { padding: '0 0 0 0' },

  toolbar: {
    display: 'flex', alignItems: 'center', gap: 6,
    padding: '10px 16px',
    borderTop: '1px solid var(--color-accent-soil)',
    flexWrap: 'wrap',
  },
