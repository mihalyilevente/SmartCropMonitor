 ...
 * Endpoints used:
 *   GET /api/v1/events/user/{user_id}          → active events (already used by AlertsPanel)
 *   GET /api/v1/weather/charts/{location_id}   → hourly forecast (already fetched in Dashboard)
 *
 */
...
// ── Spraying suitability from hourly forecast ─────────────────────────────────
// Returns array of next-24h hourly slots with a simple GO / HOLD label.
// Criteria mirror SprayingWindowsPanel: wind < 3.5 m/s, rain < 0.1 mm/h.

const assessSprayingHours = (chartData) => {
  ...
  return chartData
    .filter(h => {
      const t = new Date(h.timestamp || h.dt || h.time);
      return t >= now && t <= cutoff;
    })
    .map(h => {
      const wind = h.wind_speed ?? h.ws ?? null;
      const rain = h.rain ?? h.r ?? 0;
      const temp = h.temp ?? h.t ?? null;
      const ts   = new Date(h.timestamp || h.dt || h.time);

      const ok =
        wind !== null && wind < 3.5 &&
        rain < 0.1 &&
        temp !== null && temp > 5 && temp < 28;
...
const SpraySlot = ({ slot }) => (
  <div style={{
    display: 'flex', flexDirection: 'column', alignItems: 'center',
    minWidth: 54, padding: '7px 6px', borderRadius: 8,
    background: slot.ok ? '#e8f5e9' : '#fce4ec',
    border: `1px solid ${slot.ok ? '#a5d6a7' : '#ef9a9a'}`,
  }}>
    <span style={{ fontSize: 11, fontWeight: 700, color: '#555' }}>{slot.label}</span>
    <span style={{ fontSize: 15, marginTop: 2 }}>{slot.ok ? '✅' : '🚫'}</span>
    <span style={{ fontSize: 9, color: '#999', marginTop: 2 }}>💨{slot.wind}</span>
    <span style={{ fontSize: 9, color: '#999' }}>🌧{slot.rain}</span>
  </div>
);

// ── Main component ────────────────────────────────────────────────────────────

const MorningBriefingPanel = ({ userId, locationId, chartData = [] }) => {
  ...
  // Active events only, sorted by severity then event_type
  const active = events
  ...
  const spraySlots  = assessSprayingHours(chartData);
  const goodSlots   = spraySlots.filter(s => s.ok).length;
          ...
          {loading ? (
            ...
            <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap', alignItems: 'flex-start' }}>
              ...
              <div style={{ flex: '1 1 300px', minWidth: 260 }}>
                <div style={sectionLabel}>📋 Field risk ranking</div>
                ...
                <div style={{
                  ...
                  {active.length > 0 && (
                    <span style={{ fontWeight: 400, color: '#888', marginLeft: 6 }}>
                      {highCount > 0 ? `${highCount} high · ` : ''}{mediumCount > 0 ? `${mediumCount} medium · ` : ''}{active.length} total active
                    </span>
                  )}
                </div>
              ...
              {/* Right: 24h spraying window */}
              <div style={{ flex: '1 1 260px', minWidth: 240 }}>
                <div style={sectionLabel}>
                  🌬️ Spraying window — next 24h
                  {spraySlots.length > 0 && (
                    <span style={{ marginLeft: 8, fontSize: 11, fontWeight: 400, color: '#aaa' }}>
                      {goodSlots}/{spraySlots.length} hours suitable
                    </span>
                  )}
                </div>
                ...
                {spraySlots.length === 0 ? (
                  ...
                  <>
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
                      {spraySlots.map((s, i) => (
                        <SpraySlot key={i} slot={s} />
                      ))}
                    </div>
                    <div style={{ fontSize: 11, color: '#aaa', marginTop: 4 }}>
                      ✅ = wind &lt;3.5 m/s, rain &lt;0.1 mm/h, temp 5–28°C
                    </div>
                  </>
                )}
              </div>
...
const sectionLabel = { fontSize: 10, fontWeight: 700, color: '#aaa', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 10 };